...
<body class="font-Satoshi bg-white dark:bg-zinc-800">
    <div class="grid grid-cols-12 grid-rows-6 h-screen max-w-screen-1366 m-auto">
        <!-- Header -->
        <header class="flex items-center px-8 col-start-3 col-end-13 h-14 md:h-16 justify-between shadow shadow-slate-200 dark:shadow max-w-screen-1366">
            <!-- Search Bar -->
        ...
        <!-- Barra lateral -->
        <aside class="col-span-2 row-span-full border-l border-r border-slate-200 dark:border-zinc-700 max-h-screen">
        ...
        <main class="rounded-xl -mt-5 mb-6 mx-6 col-start-3 col-end-13 row-start-2 row-span-full grid grid-cols-4 grid-rows-8">
            <div class="row-span-1 col-span-full flex items-center justify-between py-2">
                <h3 class="text-4xl font-medium text-slate-600 dark:text-slate-200">Lista de Filmes!</h2>
                <button class="bg-blue-500 hover:bg-blue-600 text-white font-semibold py-1 px-4 rounded-md cursor-pointer"><a href="../filmes/createFilme.php">Adicionar Filme</a></button>
            </div>
                        ...
                        if ($result->num_rows > 0) {
                            ...
                            while ($row = $result->fetch_assoc()) {
                                echo "<tr class='odd:bg-white bg-slate-100 odd:dark:bg-zinc-600 dark:bg-zinc-700 dark:border-gray-700 hover:bg-zinc-100 hover:dark:bg-zinc-500 [&>td]:px-6 [&>td]:py-4 [&>td]:font-medium [&>td]:text-gray-900 [&>td]:dark:text-white [&>td]:whitespace-nowrap'>" .
                                        ...
                                        "<td scope='row'>" . $row["ano_lançamento"] . "</td>" .
                                        "<td scope='row'>" . $row["duração"] . " minutos</td>" .
                                        "<td scope='row'>" . $row["género"] . "</td>" .
                                        "<td scope='row'>" . $row["estudio_id"] . "</td>" .
                                        "<td scope='row'>
                                            <a href='../filmes/updateFilme.php?id=$row[id]'>
                                                <i class='fa-solid fa-pen-to-square self-center px-2 text-green-500'></i>
                                            </a>
                                            <a href='../filmes/deleteFilme.php?id=$row[id]'>
